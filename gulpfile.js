...
var Config = {
  ...
  banners: {
    unminified:
      ...
      " *\n" +
      " * Generated at " +
      gutil.date(today, "dddd, mmmm dS, yyyy, h:MM:ss TT") +
      "\n" +
      " */",
    minified:
      "/*! " +
...
gulp.task("dist:js:clean", function () {
  return gulp
    .src([Config.paths.compileMinified.root + "/**/*.js"], { read: false })
    .pipe(clean());
});
gulp.task("dist:js", gulp.series("dist:js:clean", "scripts"), function () {
  return gulp
    .src(Config.paths.compileUnminified.js + "/**/*.js")
    .pipe(uglify())
...
gulp.task("server", function () {
  express().use(express_lr()).use(express.static(".")).listen(Config.port);
...
gulp.task("minify", function () {
  return gulp
    .src("compile/unminified/svg-morpheus.js")
    .pipe(uglify())
    .pipe(header(Config.banners.minified))
    .pipe(gulp.dest("compile/minified"));
});

// Build
gulp.task("build", gulp.series("dist:js", "minify"));

// Start server and watch for changes
gulp.task(
  "default",
  gulp.series("server", "livereload", "scripts", "watch"),
  function () {
